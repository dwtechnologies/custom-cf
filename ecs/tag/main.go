...
type config struct {
	...
	resourceProperties    *ResourceTags // The new resource data from the template.
	oldResourceProperties *ResourceTags // The old resource data, only on updates.
}
...
func createConfig(ctx context.Context, req *events.Request) *config {
	return &config{
		...
		resourceProperties:    &ResourceTags{},
		oldResourceProperties: &ResourceTags{},
	}
}
